...
3. Per-host pipeline (all hosts run in parallel, semaphore-bounded):
   ...
   - **Phase: vulnscan** — `scan_host_vulns` (nuclei + nmap `--script=vuln` + searchsploit + getsploit + nikto + **NVD CVE search pass 1** [ports+tech only]) ∥ `scan_msf` (Metasploit auxiliary/scanner modules, service-routed) run in parallel via `asyncio.gather`. **NVD CVE search pass 2** runs in the orchestrator after enumeration — it re-queries the NVD API with full findings (including versions from enum4linux, NetExec, SNMP, TLS, LDAP, CMS, and service_enum) and deduplicates against existing CVEs before appending new ones.
...
| `src/wireghost/config.py` | `ScanConfig` with layered loader (defaults → YAML → env → overrides). Key flags: `enhanced_discovery` (TCP 17 ports + UDP 5 ports, default True), `skip_passive_dns`, `scan_unresponsive`, `skip_msf_scan`, `skip_fingerprintx`, `vulners_api_key`, `nvd_api_key` |
...
| `src/wireghost/pipeline/vulnscan.py` | Phase: vulnscan — nuclei + nmap vuln + searchsploit + getsploit + nikto + NVD CVE pass 1 (runs in parallel with msf_scan via `asyncio.gather`) |
| `src/wireghost/pipeline/cve_search.py` | NVD CVE search — CPE 2.3 URI builder, ~200-entry CPE vendor/product map, version extraction from 11 sources (nmap -sV, httpx tech detect, SMB/NetExec/SMTP/SSH/TLS/SNMP/LDAP/MySQL/PostgreSQL banners, service_enum), NVD API 2.0 client with rate limiting (5 req/30s without key, 50 with), two-pass architecture: pass 1 in vulnscan (ports+tech), pass 2 in orchestrator (full findings, deduplicated). Entry point: `scan_cve_search(host, findings, config)` |
...
| `web_portal/scanner/tasks/__init__.py` | Celery bridge: ORM → `run_pipeline` → ORM; AD recon 8-phase task imported from `tasks/ad_recon.py`; artifact queue handler (inline in `_progress_thread`) — resolves host FK by scan+ip and creates ScanArtifact records |
| `web_portal/scanner/models/__init__.py` | Django models — User, Scan (with `current_phase`, `hosts_scanned`, `hosts_total` for real-time progress), Host (with `current_phase` for per-host phase tracking, migration 0031), Port, Finding, Asset, Permission, ScanPolicy, ScheduledScan, ApiToken, ReportConfig, SiteConfig, ExploitMatch, AuditLog, Screenshot, ScanArtifact (raw tool output persistence — nmap XML, nuclei JSON, enum4linux txt — with FK to Scan+Host, migration 0037); 11 encrypted AD recon models re-exported from `models/ad_recon.py` |
| `web_portal/scanner/views/__init__.py` | DRF ViewSets with RBAC; AD recon ViewSet in `views/ad_recon.py`; ScanArtifactViewSet (read-only, `?scan=/?host=/?tool=` filters) |
| `web_portal/scanner/serializers/__init__.py` | DRF serializers; AD recon serializers in `serializers/ad_recon.py`; ScanArtifactListSerializer (excludes content) + ScanArtifactSerializer (full detail), switched via `get_serializer_class()` |
...
| `web/js/app/pages/host-detail.js` | Host detail page — tabs for Ports, Findings, Technologies, Screenshots, Artifacts; accordion-based artifact viewer with lazy content loading |
| `web/js/app/pages/finding-detail.js` | Finding detail page — CVSS, CVE/CWE, HTTP request/response, curl reproduce, Host Artifacts section |
| `web/js/app/pages/live-discovery.js` | Live Host Discovery tab — 2-second polling during discovery phase, real-time host table |
| `web/js/app/pages/phase-view.js` | Shared phase dashboard — renders per-phase progress for any pipeline phase |
...
- Pytest: `asyncio_mode = "auto"` — async tests don't need `@pytest.mark.asyncio`. ~890 tests collected. Django test runner (CI) reports 623 tests.
...
- **NVD CVE search is two-pass**: Pass 1 runs inside `scan_host_vulns()` (vulnscan phase) with only `host.ports` and `host.technologies` for version extraction — this catches CVEs from nmap -sV and httpx tech detect. Pass 2 runs in the orchestrator's `_host_pipeline()` after enumeration, passing the full findings list to `scan_cve_search(host, findings, config)` so it extracts versions from enum4linux, NetExec, SNMP, TLS, LDAP, CMS, and service_enum banners. Pass 2 deduplicates against CVEs already in findings before appending. The CPE map in `_CPE_VENDOR_MAP` (~200 entries across 12 categories) maps lowercase product names → (vendor, product) tuples for CPE 2.3 URI construction (`cpe:2.3:a:<vendor>:<product>:<version>:*:*:*:*:*:*:*`). NVD API 2.0 is rate-limited: 5 req/30s without key, 50 with `nvd_api_key`. `_query_nvd_cpe()` uses `await session_obj.close()` in a finally block — mock tests must use `AsyncMock()` for `.close()` and `__aenter__`.
- **ScanArtifact queue handler is unimportable**: The artifact creation logic lives inline inside `_progress_thread()` in `scanner/tasks/__init__.py` — it's a local function within a daemon thread, not a module-level callable. Tests must replicate the ORM logic (host lookup by `scan_id + ip`, `ScanArtifact.objects.create()` with `size=len(content)`) via a helper. Don't try to import `_handle_queue_item` — it doesn't exist at module scope.
- **ScanArtifact serializer switching**: `ScanArtifactViewSet.get_serializer_class()` returns `ScanArtifactListSerializer` for `self.action == "list"` (excludes `content` for lightweight payloads) and `ScanArtifactSerializer` for `retrieve` (includes `content`). The list endpoint at `/api/artifacts/?host=<uuid>` is ~10 KB for 50 artifacts; the detail at `/api/artifacts/<uuid>/` can return megabytes of raw tool output.
- **Artifact emission from orchestrator**: `_emit_dir_artifacts(ip, directory, tool)` reads text files (filtered by `_TEXT_EXT` — `.xml`, `.json`, `.txt`, `.html`, `.csv`, `.log`, `.nmap`, `.gnmap`, `.yml`, `.yaml`) and calls `on_artifact(host_ip, tool, filename, content, content_type)`. Called after portscan (nmap XML dir), vulnscan (vuln dir), and enumeration (host dir top-level). The callback uses `put_nowait()` with silent `except Exception: pass` — non-blocking but can silently drop artifacts under extreme backpressure.
...
- **Updater full recreate is destructive to running scans**: `scripts/update-wireghost.sh` does `docker compose down --remove-orphans` followed by `build --no-cache --pull` and `up -d --force-recreate`. All containers are destroyed and rebuilt — running scans, in-flight Celery tasks, and in-memory state are lost. DB data survives (volume). Wait for active scans to finish before updating. The build step can take 5-10 minutes on first run (pulls all base images, compiles Metasploit/nuclei).
- **`makemigrations` must run on the HOST, not in the API container**: The API image (`callmedemon/wireghost:web`) uses a read-only rootfs. `python manage.py makemigrations` tries to write to the filesystem and fails with `OSError: [Errno 30] Read-only file system`. Create migration files on the host at `web_portal/scanner/migrations/` and apply them with `docker compose exec api python manage.py migrate` (which only writes to the DB, not the filesystem).
- **Docker DNS caches stale container IPs**: When an API container is recreated (new IP), the nginx/portal container may still resolve the old IP for several minutes due to Docker's embedded DNS TTL. This produces 502 Bad Gateway even though the API container is healthy. Fix: `docker compose restart portal` to force a fresh DNS resolution. Don't recreate the API container again — the issue is nginx's cached IP, not the API.
- **Django index name migrations use RemoveIndex + AddIndex**: When renaming a database index, create both operations in a single migration (not two separate migrations). Django's `makemigrations` detects the mismatch between the model's `models.Index(name="new_name")` and the DB's existing index name, generating `RemoveIndex` for the old name and `AddIndex` for the new name. This is atomic — the old index is dropped and the new one created in one `migrate` call.
