...
| `src/wireghost/pipeline/discovery.py` | Phase: discovery — nmap/fping/ARP/DNS sweep with per-subnet parallel tool execution, CIDR partitioning (multi-target comma-separated support with dedup), fping ICMP Host Unreachable capture (firewalled hosts always scanned), dedicated 256-concurrency semaphore (`max(config.parallelism, 256)`), returns `(sorted_ips, mac_vendor, dns_hostnames)` |
| `src/wireghost/pipeline/portscan.py` | Phase: portscan — port discovery fallback chain (nmap → naabu → masscan) + service analysis (nmap -sV -sC -O) + fingerprintx augmentation. Masscan uses `--rate 5000` (~13s per host); without --rate it defaults to 100 pps (~11 min per host) causing event-loop deadlocks |
