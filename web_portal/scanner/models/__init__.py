...
class ExploitMatch(models.Model):
    ...
    port = models.ForeignKey(
        Port, on_delete=models.SET_NULL, null=True, blank=True, related_name="exploit_matches"
    )
    finding = models.ForeignKey(
        Finding, on_delete=models.SET_NULL, null=True, blank=True, related_name="exploit_matches"
    )
...
class AuditLog(models.Model):
    ...
    def log(cls, user, action, detail="", log_type="admin", ip=None):
        cls.objects.create(
            user=str(user), action=action, detail=detail, type=log_type, ip_address=ip
        )
