...
async def handle_list(query, user, rest, context):
    ...
    def _q():
        ...
        rows = list(
            ...
            .values_list(
                'id', 'ip', 'hostname', 'service_name', 'risk_score',
                'findings_count', 'critical_count', 'high_count', 'last_seen',
            )
        )
...
async def handle_detail(query, user, rest, context):
    ...
    if not asset:
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⬅ Assets', callback_data='al:0')]]),
            parse_mode=HTML,
        )
        return
