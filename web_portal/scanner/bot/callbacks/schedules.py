...
async def handle_list(query, user, rest, context):
    ...
    def _q():
        ...
        rows = list(
            ...
            .values_list('id', 'name', 'target', 'enabled', 'frequency', 'time',
                         'next_run', 'scan_type')
        )
...
async def handle_detail(query, user, rest, context):
    ...
    if not sched:
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⬅ Schedules', callback_data='cl:0')]]),
            parse_mode=HTML,
        )
        return
...
async def handle_run_now(query, user, rest, context):
    ...
    if not sched:
        await query.edit_message_text(
            f'Schedule <code>{esc(sid)}</code> not found.', parse_mode=HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⬅ Schedules', callback_data='cl:0')]]),
        )
        return
