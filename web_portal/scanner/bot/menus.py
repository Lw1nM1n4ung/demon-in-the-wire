    ...
    rows = [
        [
            InlineKeyboardButton(
                "🔴 Critical", callback_data=f"{prefix}:critical" if scan_sid else "fl:critical:0"
            ),
            InlineKeyboardButton(
                "🟠 High", callback_data=f"{prefix}:high" if scan_sid else "fl:high:0"
            ),
        ],
        [
            InlineKeyboardButton(
                "🟡 Medium", callback_data=f"{prefix}:medium" if scan_sid else "fl:medium:0"
            ),
            InlineKeyboardButton(
                "🔵 Low", callback_data=f"{prefix}:low" if scan_sid else "fl:low:0"
            ),
        ],
...
def findings_list_kb(
    page: int,
    total_pages: int,
    severity: str = "all",
    scan_sid: str | None = None,
    finding_buttons: list | None = None,
...
def _pagination_row_ext(
    prefix: str, page: int, total_pages: int, extra: str
