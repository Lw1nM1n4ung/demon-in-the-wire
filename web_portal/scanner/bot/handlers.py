...
from scanner.bot.menus import scan_detail_kb
...
async def cmd_unlock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    def _do_unlock():
        ...
        AuditLog.log(
            user.username,
            "auth.unlock",
            f"Account unlocked via Telegram (tg_user={tg_user.id})",
            "auth",
        )
        return token
...
async def cmd_scans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    def _query():
        return list(
            Scan.objects.order_by("-created_at")[:10].values_list(
                "id",
                "name",
                "target",
                "scan_type",
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not scan:
        await update.message.reply_text(
            f"Scan <code>{esc(prefix)}</code> not found.", parse_mode=HTML
        )
        return
...
async def cmd_assets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    def _query():
        return list(
            Asset.objects.order_by("-risk_score")[:10].values_list(
                "ip",
                "hostname",
                "service_name",
                "risk_score",
    ...
    for ip, hostname, svc, risk, fcount, ports, last_seen in rows:
        ...
        lines.append(
            f"\n  ⚠️ <code>{esc(ip)}</code>" + (f" <i>{esc(hostname)}</i>" if hostname else "")
        )
        lines.append(
...
async def cmd_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sub == "list":
        ...
        if freq not in ("daily", "weekly", "biweekly", "monthly"):
            await update.message.reply_text(
                "Invalid frequency. Use: daily, weekly, biweekly, monthly"
            )
            return
        ...
        if len(context.args) < 2:
            await update.message.reply_text(
                "Usage: <code>/schedule del &lt;id&gt;</code>",
                parse_mode=HTML,
            )
            return
        ...
        await update.message.reply_text(
            "Usage: <code>/schedule</code> list │ add │ del",
            parse_mode=HTML,
        )
...
async def cmd_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text(
            "Usage: <code>/report &lt;scan_id&gt;</code>",
            parse_mode=HTML,
        )
        return
    ...
    if not scan:
        await update.message.reply_text(
            f"No completed scan found with ID <code>{esc(prefix)}</code>.",
            parse_mode=HTML,
        )
        return
