...
def run_discovery_scan(self, scan_id):
    ...
    try:
        ...
        def _on_discovery_complete(live_ips, mac_vendor_map, dns_hostnames):
            try:
                _progress_queue.put_nowait(
        ...
        def _wrap_subnet(new_ips, mac_updates):
            if _is_aborted():
                raise _TaskCancelled("Task cancelled")
            _on_subnet_complete(new_ips, mac_updates)

        live_ips: list = []
        mac_vendor_map: dict = {}
        dns_hostnames: dict = {}
        try:
            live_ips, mac_vendor_map, dns_hostnames = asyncio.run(
                discover_hosts(
                    config,
                    tree,
            ...
            if not _is_aborted():
                _on_discovery_complete(live_ips, mac_vendor_map, dns_hostnames)
