...
(function() {
  ...
  function _buildMethodBadge(method) {
    if (!method) return '<span style="color:var(--text-dim);font-size:0.75rem;">&mdash;</span>';
    var styles = {
      nmap: 'background:rgba(59,130,246,0.15);color:#60a5fa;',
      fping: 'background:rgba(52,211,153,0.15);color:#34d399;',
      fping_unreachable: 'background:rgba(251,191,36,0.15);color:#fbbf24;',
      arp: 'background:rgba(168,85,247,0.15);color:#a855f7;',
      netdiscover: 'background:rgba(236,72,153,0.15);color:#ec4899;',
      'passive-dns': 'background:rgba(251,146,60,0.15);color:#fb923c;',
    };
    var label = method;
    if (method === 'fping_unreachable') label = 'unreachable';
    else if (method === 'passive-dns') label = 'DNS';
    var style = styles[method] || 'background:var(--bg-secondary);color:var(--text-dim);';
    return '<span style="font-size:0.6rem;padding:1px 6px;border-radius:3px;' + style + '">' + label + '</span>';
  }
  ...
  function _buildHostRow(h) {
    ...
    return '<td><code>' + esc(h.ip) + '</code></td>' +
           ...
           '<td style="white-space:nowrap">' + _buildMethodBadge(h.discovery_method) + '</td>' +
  ...
  WG.renderLiveDiscovery = function() {
    ...
    Promise.resolve(scansPromise).then(function(data) {
      ...
      allDiscovery.forEach(function(scan) {
        ...
        cardsHtml += '<thead><tr><th>IP Address</th><th>Hostname</th><th>MAC</th><th>Vendor</th><th>Method</th><th>Discovered By</th><th>Phase</th></tr></thead>';
        cardsHtml += '<tbody id="discovery-hosts-' + scan.id + '">';

        if (isRunning && !hosts.length) {
          cardsHtml += '<tr><td colspan="7" style="text-align:center;color:var(--text-dim);padding:20px"><div class="spinner" style="margin:0 auto 8px"></div>Waiting for hosts to be discovered...</td></tr>';
        } else if (!hosts.length) {
          cardsHtml += '<tr><td colspan="7" style="text-align:center;color:var(--text-dim);padding:20px">No hosts found</td></tr>';
        } else {
          hosts.forEach(function(h) {
            var tools = _toolList(h);
      ...
      if (!existing) {
        ...
        if (oldTools !== toolsAttr) {
          ...
          if (cells.length >= 7) {
            ...
            cells[4].innerHTML = _buildMethodBadge(h.discovery_method);
            cells[5].innerHTML = _toolBadges(tools);
          }
        }
      }
    });
