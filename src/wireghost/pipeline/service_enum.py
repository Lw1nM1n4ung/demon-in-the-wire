...
async def _check_ssh(ip: str, port: int) -> list[Finding]:
    ...
    return [
        _f(
            ip,
            port,
            f"SSH: {banner.split(chr(10))[0][:60]}",
            Severity.INFO,
...
async def _check_ftp(ip: str, port: int) -> list[Finding]:
    ...
    try:
        ...
        await writer.drain()
        resp = (await asyncio.wait_for(reader.read(1024), timeout=_TIMEOUT)).decode(
            errors="replace"
        )
        if "230" in resp:
            findings.append(
                _f(
                    ...
                    Severity.HIGH,
                    "FTP allows anonymous access without credentials.",
                    resp,
                )
            )
    except Exception:
        ...
        if info and b"redis_version" in info:
            findings.append(
                _f(
                    ip,
                    port,
                    "Redis: Server Info Exposed",
                    Severity.MEDIUM,
                    "Redis INFO accessible.",
                    info.decode(errors="replace")[:2000],
                )
            )
        ...
        if auth_type == 0:
            return [
                _f(
                    ip,
                    port,
                    "PostgreSQL: No Password Required (trust auth)",
                    Severity.CRITICAL,
            ...
            return [
                _f(
                    ip,
                    port,
                    "PostgreSQL: Cleartext Password Auth",
                    Severity.MEDIUM,
            ...
            return [
                _f(ip, port, "PostgreSQL: MD5 Auth", Severity.INFO, "PostgreSQL uses MD5 auth.")
            ]
        elif auth_type == 10:
...
async def _check_smtp(ip: str, port: int) -> list[Finding]:
    ...
    try:
        ...
        await writer.drain()
        ehlo = (await asyncio.wait_for(reader.read(2048), timeout=_TIMEOUT)).decode(
            errors="replace"
        )
        if "VRFY" in ehlo:
            findings.append(
                _f(
                    ip,
                    port,
                    "SMTP: VRFY Supported (user enumeration)",
                    Severity.MEDIUM,
                    "SMTP supports VRFY command.",
                    ehlo,
                )
            )
        if "EXPN" in ehlo:
            findings.append(
                _f(
                    ip,
                    port,
                    "SMTP: EXPN Supported (list enumeration)",
                    Severity.MEDIUM,
                    "SMTP supports EXPN command.",
                    ehlo,
                )
            )
    except Exception:
...
async def _check_elasticsearch(ip: str, port: int) -> list[Finding]:
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                f"http://{ip}:{port}/", timeout=aiohttp.ClientTimeout(total=_TIMEOUT)
            ) as r:
                if r.status == 200:
                    body = await r.text()
                    if "cluster_name" in body or "tagline" in body:
                        return [
                            _f(
                                ...
                                Severity.HIGH,
                                "Elasticsearch API accessible without auth.",
                                body[:500],
                            )
                        ]
    except Exception:
...
async def _check_docker(ip: str, port: int) -> list[Finding]:
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                f"http://{ip}:{port}/version", timeout=aiohttp.ClientTimeout(total=_TIMEOUT)
            ) as r:
                if r.status == 200:
                    body = await r.text()
                    if "ApiVersion" in body or "Version" in body:
                        return [
                            _f(
                                ...
                                Severity.CRITICAL,
                                "Docker API without auth. Container escape → host RCE.",
                                body[:500],
                            )
                        ]
    except Exception:
